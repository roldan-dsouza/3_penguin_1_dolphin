...
function ReaderContent() {
    ...
    if (!id || !originalText) {
        return (
            <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: preset.bg, color: preset.text }}>
    ...
    return (
        <div className="min-h-screen transition-colors duration-400" style={{ backgroundColor: preset.bg, color: preset.text }}>
            {/* Navbar */}
            <nav className="sticky top-0 z-40 backdrop-blur-md border-b" style={{ borderColor: `${preset.text}15`, backgroundColor: `${preset.bg}dd` }}>
            ...
            {/* Reader Mode Info */}
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto px-6 py-3">
                <div className="flex items-center gap-2 text-sm opacity-50">
                    <BookOpen className="w-4 h-4" />
                    <span>
                        {preferences.readingMode === "paragraph" ? "Paragraph Mode — hover a line to highlight it" : "Line-by-Line Mode — use arrow keys or buttons to navigate"}
                    </span>
    ...
    return (
        <Suspense fallback={
            <div className="min-h-screen flex items-center justify-center" style={{ background: "#FDF5E6" }}>
                <div className="animate-pulse text-2xl font-semibold" style={{ color: "#1E1E1E" }}>Loading...</div>
            </div>
        }>
            <ReaderContent />
        </Suspense>
